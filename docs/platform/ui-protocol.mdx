...
If the standard components don't fit your needs, you can still deliver completely custom UI elements.

Because the default format for Charm is Markdown, and the Store's Markdown renderer natively supports **HTML and SVG elements** (safely sanitized via DOMPurify), your Agent can generate and return raw HTML/SVG strings as its text output.

```html
<!-- Example: Custom SVG Visualization returned as a string -->
<svg width="200" height="50" xmlns="http://www.w3.org/2000/svg">
  <rect width="200" height="50" rx="10" fill="#4CAF50"/>
  <text x="100" y="30" font-family="sans-serif" font-size="16" fill="white" text-anchor="middle">
    System Operational
  </text>
</svg>
```

> **Security Note:** The Charm Store frontend uses DOMPurify to strip `<script>`, `<iframe>`, and other potentially malicious vectors. Keep your custom UI elements constrained to static HTML, CSS classes (Tailwind is supported on the Store), and SVGs.

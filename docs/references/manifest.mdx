...
summary: The complete UAC Manifest Schema Reference
---
...
runtime:
  ...
  # --------------------------------------------------------------
  capabilities:
    # If True, declares that this agent/adapter manages its own persistent conversation
    # history and state (e.g. SQLite, local memory files, continuous daemon sessions).
    # Charm Store and Desktop mini-chat will hide external thread-switching and treat it as stateful.
    manages_own_history: false
...
| |custom_image |String |Custom Docker image URI for the runtime environment.|
| |memory |Object |Configuration for persistent memory and state storage (e.g., provider).|
| |telemetry |List |List of telemetry exporter names to enable.|
| |mode |Enum |standard (Default), full. Selects the container environment (Deprecated).|

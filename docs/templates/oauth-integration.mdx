...
summary: Skeleton documentation for agents that require user-authorized OAuth connections.
---

There is **no** `charm init --template oauth-integration` starter in the CLI. OAuth is configured in `charm.yaml` with `auth.providers` on top of any scaffold template (for example `python` or `openclaw`).
